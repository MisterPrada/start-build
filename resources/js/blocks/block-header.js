import { breakpointResizer } from '../utils/globals'

(function ($) {
	const sectionSelector = '.header'
	...
	const bottomNavigationModule = function( $header ) {
	...
	const initializeBlock = function () {
		const headerWrap = $( sectionSelector ).parents('.wp-block-template-part');
		...
		$( sectionSelector ).each( function() {
			const $header = $( this )
			const search = searchModule( $header )
			const fullscreenMenu = fullscreenMenuModule( $header )
